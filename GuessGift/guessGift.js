...
Input: array of whish list and another array with the presents
Output: array of presents I have gotten
    ...
    for (let wish of wishlist) {
        for (let present of presents) {
            if (wish.size === present.size && wish.clatters === present.clatters && wish.weight === present.weight){
                guessedGift.push(wish.name);
            }
        }
    }
    return [...new Set(guessedGift)];
}
